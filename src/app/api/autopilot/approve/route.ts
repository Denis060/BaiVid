...
export async function GET(request: Request) {
  ...
  if (!token) {
    return new NextResponse(renderPage("Invalid Link", "No approval token provided."), {
      status: 400,
  ...
  if (!approval) {
    return new NextResponse(renderPage("Not Found", "This approval link is invalid or has already been used."), {
      status: 404,
      headers: { "Content-Type": "text/html" },
    });
  }
  ...
  if (approval.expires_at && new Date(approval.expires_at) < new Date()) {
    return new NextResponse(renderPage("Expired", "This approval link has expired (2 hour limit)."), {
      status: 410,
      headers: { "Content-Type": "text/html" },
    });
  }
  ...
  // Send Inngest event to resume the pipeline
  await inngest.send({
  ...
  return new NextResponse(
    renderPage("Approved!", "Your autopilot content has been approved and will be published shortly."),
